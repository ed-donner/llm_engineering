...
class Tester:
    ...
    def post_process(value):
        if isinstance(value, str):
            # First try: find dollar-prefixed amounts (most reliable for CoT outputs)
            dollar_matches = re.findall(r"\$\s*([\d,]+\.?\d*)", value)
            if dollar_matches:
                return float(dollar_matches[-1].replace(",", ""))
            # Fallback: strip symbols and grab the last number
