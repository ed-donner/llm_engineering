...
class TokenPredictor_DeepSeek:
    def __init__(self, model_name: str):
        """
        Initialize the TokenPredictor with DeepSeek API.
        
        Args:
            model_name: The model name (e.g., "deepseek-chat" or "deepseek-coder")
            api_key: Your DeepSeek API key (optional, can be set in .env as DEEPSEEK_API_KEY)
            base_url: DeepSeek API base URL (defaults to DeepSeek's endpoint)
        """
        # Use DeepSeek's API endpoint
        self.client = OpenAI(
    ...
    def predict_tokens(self, prompt: str, max_tokens: int = 100) -> List[Dict]:
        ...
        for chunk in response:
            if chunk.choices[0].delta.content:
                ...
                # DeepSeek returns logprobs in the same format as OpenAI
                if chunk.choices[0].logprobs and chunk.choices[0].logprobs.content:
                    ...
                    top_prob = logprob_dict.get(token, -float('inf'))
                    ...
                    prediction = {
                        "token": token,
                        "probability": math.exp(top_prob) if top_prob > -float('inf') else 0,
...
def create_token_graph_DeepSeek(model_name: str, predictions: List[Dict]) -> nx.DiGraph:
    """
    ...
    elif predictions:  # If there are predictions but no alternatives
        G.add_edge(f"t{len(predictions)-1}", "END")

    return G


def visualize_predictions_DeepSeek(G: nx.DiGraph, figsize=(14, 80)):
    """
    ...
    plt.title("Token prediction with DeepSeek")
    plt.axis("off")
