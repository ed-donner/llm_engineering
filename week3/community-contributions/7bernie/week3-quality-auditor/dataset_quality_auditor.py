...
def build_report(reference_path: Path, synthetic_path: Path) -> str:
    ...
    for col in common_cols:
        ...
        report.append(
            f"| `{col}` | {_format_float(ref_missing)} | {_format_float(syn_missing)} | "
    ...
    if numeric_cols:
        ...
        for col in numeric_cols:
            ...
            report.append(
                f"| `{col}` | {_format_float(metrics['ref_mean'])} | {_format_float(metrics['syn_mean'])} | "
    ...
    if categorical_cols:
        ...
        for col in categorical_cols:
            ...
            report.append(f"| `{col}` | {_format_float(float(tvd))} |")
        report.append("")
