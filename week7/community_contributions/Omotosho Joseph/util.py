"""
Evaluation utility for the Salary Is Right — Week 7 (QLoRA fine-tuning).
Designed to run on Google Colab where data comes from HuggingFace Datasets
in prompt/completion dict format.
"""
...
class Tester:
    ...
    def run_datapoint(self, i):
        ...
        pieces = datapoint["prompt"].split("Job Title: ")
        ...
        fig.add_trace(
            go.Scatter(
                ...
                hovertemplate=(
                    "n=%{x}<br>"
                    "Avg Error=$%{y:,.0f}<br>"
                    "±95% CI=$%{customdata[0]:,.0f}<extra></extra>"
                ),
            )
        )
        fig.update_layout(
    ...
    def report(self):
        ...
        title = f"{self.title} results<br><b>Error:</b> ${average_error:,.0f} <b>MSE:</b> {mse:,.0f} <b>r²:</b> {r2:.1f}%"
