...
from llm_engineering.week5.evaluation.test import TestQuestion, load_tests
from rag import answer_question, fetch_context


load_dotenv(override=True)

OPEN_ROUTER_EVAL_MODEL = os.getenv(
    "OPEN_ROUTER_EVAL_MODEL",
    os.getenv("OPEN_ROUTER_CHAT_MODEL", "openai/gpt-4.1-nano"),
)
OPENAI_EVAL_MODEL = os.getenv("OPENAI_EVAL_MODEL", "gpt-4.1-nano")
OLLAMA_EVAL_MODEL = os.getenv("OLLAMA_EVAL_MODEL", "gemma4:e2b")
litellm.ssl_verify = False

def to_openrouter_litellm_model(model_name: str) -> str:
    """Ensure LiteLLM receives an explicit provider prefix 'openrouter/'."""
    return model_name if model_name.startswith("openrouter/") else f"openrouter/{model_name}"


def resolve_eval_llm_config() -> dict[str, Any]:
    """Resolve provider-specific LiteLLM completion settings for answer evaluation."""
    ...
    if provider == "openrouter":
        return {
            ...
            "model": to_openrouter_litellm_model(OPEN_ROUTER_EVAL_MODEL),
            "ssl_verify": False, # Disable SSL verification for local testing with self-signed certs; use CA bundle in production
        }

    if provider == "ollama":
        model_name = OLLAMA_EVAL_MODEL
        return {
            "model": model_name if model_name.startswith("ollama/") else f"ollama/{model_name}",
            "api_base": os.getenv("OLLAMA_API_BASE", "http://localhost:11434"),
        }

    raise ValueError("Unsupported LLM_PROVIDER. Use: openai, openrouter, ollama")
...
def generate_answer(question: str) -> tuple[str, list]:
    """Generate an answer for evaluation without prior chat history."""
    generated_answer, retrieved_docs, _ = answer_question(question, [], "")
...
def evaluate_answer(test: TestQuestion) -> tuple[AnswerEval, str, list]:
    ...
    generated_answer, retrieved_docs = generate_answer(test.question)
...
Provide detailed feedback and scores from 1 (very poor) to 5 (ideal) for each dimension. If the answer is wrong, then the accuracy score must be 1.""",
    ...
    response_content = next((field for field in candidate_fields if isinstance(field, str) and field.strip()), None)

    if not isinstance(response_content, str):
        raise ValueError(
            f"Judge response did not return JSON content in supported fields: {candidate_fields}. \n Full response: {judge_response}"
        )

    try:
        answer_eval = AnswerEval.model_validate_json(response_content)
    except Exception:
        parsed_payload = json.loads(response_content)
        answer_eval = AnswerEval.model_validate(parsed_payload)
