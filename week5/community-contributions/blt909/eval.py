...
def evaluate_retrieval(test: TestQuestion, repo: str, k: int = 10) -> RetrievalEval:
    """
    ...
    retrieved_docs = fetch_context(test.question, repo)
...
def evaluate_answer(test: TestQuestion, repo: str) -> tuple[AnswerEval, str, list]:
    """
    ...
    generated_answer, retrieved_docs = answer_question(test.question, repo)
...
def evaluate_all_retrieval(repo: str):
    """Evaluate all retrieval tests."""
    tests = load_tests()
    total_tests = len(tests)
    for index, test in enumerate(tests):
        result = evaluate_retrieval(test, repo)
        progress = (index + 1) / total_tests
        yield test, result, progress


def evaluate_all_answers(repo: str):
    """Evaluate all answers to tests using batched async execution."""
    tests = load_tests()
    total_tests = len(tests)
    for index, test in enumerate(tests):
        result = evaluate_answer(test, repo)[0]
        progress = (index + 1) / total_tests
        yield test, result, progress
