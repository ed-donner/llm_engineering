...
# AnswerEval is a sub-class of the BaseModel class
...
Provide detailed feedback and scores from 1 (very poor) to 5 (ideal) for each dimension. If the answer is wrong, then the accuracy score must be 1.""",
    ...
    # Call LLM judge with structured outputs (async) - we are using this with LiteLLM, the completion function from week 1
    judge_response = completion(
        model=MODEL, messages=judge_messages, response_format=AnswerEval
    )  # note we are passing an arg, AnswerEval to the response_format param
    # this tells the LLM we want the LLM to response in JSON and following this EXACT FORMAT. we pass in a pydantic object to convey this.
    # we define the fields in AnswerEval and give a short description for each of the fields in AnswerEval

    # then we are getting the response aka json from the LLM and converting it to a AnswerEval object
