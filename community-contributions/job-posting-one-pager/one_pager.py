...
Supports URL (scrape) or pasted text (e.g. from LinkedIn).
"""
import os
...
def get_job_text(url_or_text: str) -> str:
    """
    If url_or_text looks like a URL, fetch and return page content.
    Otherwise return it as-is (pasted job text).
    """
    s = url_or_text.strip()
...
def generate_one_pager(
    ...
    *,
    ...
    """
    Generate a one-pager from a job URL or pasted job text.

    - url_or_pasted_text: Either a job page URL or the raw pasted text of the job ad.
    - model: OpenAI model name (e.g., 'openai/gpt-4o-mini', 'anthropic/claude-3.5-sonnet').
    - client: Optional OpenAI client (creates OpenRouter client if not provided).

    Returns the one-pager as a markdown string.
    """
...
def stream_one_pager(
    ...
    *,
    ...
    """
    Stream the one-pager token-by-token (e.g. for display in notebook/UI).
    Yields text chunks.
    """
    job_text = get_job_text(url_or_pasted_text)
    if not job_text or len(job_text.strip()) < 50:
...
def main() -> None:
    """CLI: one argument = URL or pasted text; no args = read from stdin."""
    import sys
