...
class FrontierAgent(Agent):
    ...
    def price_with_confidence(self, description: str) -> tuple[float, float]:
        """
        Estimate price with confidence based on RAG similarity
        Higher similarity to retrieved items = higher confidence
        """
        documents, prices = self.find_similars(description)
        
        # Calculate confidence based on price variance of similar items
        # Lower variance = higher confidence
        if len(prices) > 1:
            import numpy as np
            price_std = np.std(prices)
            price_mean = np.mean(prices)
            # Normalize: low coefficient of variation = high confidence
            cv = price_std / price_mean if price_mean > 0 else 1.0
            confidence = max(0.5, min(0.95, 1.0 - cv))
        else:
            confidence = 0.7  # Default if not enough data
        
        # Get the actual price prediction
        self.log(
            f"Frontier Agent is about to call {self.MODEL} with context including 5 similar products"
        )
        response = self.client.chat.completions.create(
        ...
        self.log(f"Frontier Agent completed - predicting ${price:.2f} (confidence: {confidence:.2f})")
