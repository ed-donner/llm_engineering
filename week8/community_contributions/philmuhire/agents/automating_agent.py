...
from agents.pushover_agent import PushoverAgent
from openai import OpenAI
import os
import json


class AutoPlanningAgent(Agent):
    ...
    def __init__(self, collection):
        ...
        """
        self.log("Auto Planning Agent is initializing")
        self.scanner = ScannerAgent()
        self.llm = LLMAgent(collection)
        self.messenger = PushoverAgent()
        self.openai = OpenAI(api_key=os.getenv("OPENAI_API_KEY"), base_url='https://openrouter.ai/api/v1')
        self.memory = None
        self.opportunity = None
        self.log("Auto Planning Agent is ready")

    def scan_the_internet_for_bargains(self) -> str:
        """
        Run the tool to scan
        """
        self.log("Auto Planning agent is calling scanner")
    ...
    def estimate_true_value(self, description: str) -> str:
        ...
        """
        self.log("Auto Planning agent is estimating value via LLM Agent")
        estimate = self.llm.price(description)
    ...
    def notify_user_of_deal(
        ...
        """
        if self.opportunity:
            self.log("Auto Planning agent is trying to notify the user a 2nd time; ignoring")
        else:
            self.log("Auto Planning agent is notifying user")
    ...
    def plan(self, memory: List[str] = []) -> Optional[Opportunity]:
        ...
        """
        self.log("Auto Planning Agent is kicking off a run")
        self.memory = memory
        self.opportunity = None
        ...
        self.log(f"Auto Planning Agent completed with: {reply}")
