"""
Researcher Agent: gathers or simulates input for the pipeline.
"""
...
class ResearcherAgent(Agent):
    ...
    def research(self, topic: str) -> str:
        ...
        if self.use_llm and self._client:
            try:
                response = self._client.chat.completions.create(
                    model=self.MODEL,
                    messages=[
                        {"role": "system", "content": "Summarize this topic in 2-4 sentences."},
        ...
        mock = f"Research summary for '{topic}': placeholder summary. Enable use_llm=True for real results."
