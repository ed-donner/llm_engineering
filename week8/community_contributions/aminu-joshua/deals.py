...
def fetch_jumia_deals(url):
    r = requests.get(url)
    soup = BeautifulSoup(r.text, "html.parser")

    deals = []

    for item in soup.select("article.prd")[:10]:
        title = item.select_one("h3.name").text
        price = item.select_one(".prc").text
        link = "https://www.jumia.com.ng" + item.select_one("a.core")["href"]
...
class ScrapedDeal:
    ...
    @classmethod
    def fetch(cls, show_progress: bool = False, feeds: List[str] = []) -> List[Self]:
        """
        ...
        for feed_url in feed_iter:
            feed = fetch_jumia_deals(feed_url)
