"""
Launch The Price is Right Gradio app.

Ensures week8 is on path and uses winniekariuki agents (specialist + frontier).
Run from project root: python week8/community_contributions/winniekariuki/price_is_right.py
Or from week8: python community_contributions/winniekariuki/price_is_right.py
"""
import os
import sys
from pathlib import Path

# Add winniekariuki first so our agents shadow week8/agents
ROOT = Path(__file__).resolve().parent.parent.parent.parent
WEEK8 = ROOT / "week8"
WINNIE = Path(__file__).resolve().parent
sys.path.insert(0, str(WINNIE))
    ...
    def run(self):
        with gr.Blocks(title="The Price is Right", fill_width=True) as ui:
            ...
            def do_select(selected_index: gr.SelectData):
                ...
                opportunities = fw.memory
                if not opportunities:
                    return
                row = selected_index.index[0]
                if row < 0 or row >= len(opportunities):
                    return
                fw.planner.messenger.alert(opportunities[row])

            with gr.Row():
                gr.Markdown(
                    '<div style="text-align: center;font-size:24px"><strong>The Price is Right</strong></div>'
                )
            with gr.Row():
                gr.Markdown(
                    '<div style="text-align: center;font-size:14px">Fine-tuned Llama (Modal) + RAG (ChromaDB + GPT-4o-mini)</div>'
                )
            with gr.Row():
