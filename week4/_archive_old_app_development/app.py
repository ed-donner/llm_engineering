...
def stream_huggingface(python, model_name):
    ...
    try:
        ...
        # Build debug message (cumulative)
        debug_msg = ""
        if hf_token:
            debug_msg += f"🔑 Using HF token (first 10 chars): {hf_token[:10]}...\n\n"
        else:
            debug_msg += f"⚠️ No HF_TOKEN found - using public API (limited)\n\n"
        
        # Get the model ID
        model_id = HF_MODELS.get(model_name)
        if not model_id:
            yield debug_msg + f"❌ Unknown model: {model_name}"
            return
        
        debug_msg += f"📡 Calling model: {model_id}\n\n"
        debug_msg += f"⏳ Sending request to Hugging Face API...\n\n"
        yield debug_msg
        ...
        debug_msg += f"✅ Got response with status: {response.status_code}\n\n"
        
        # Check if response body is empty
        if not response.text or len(response.text.strip()) == 0:
            yield debug_msg + f"⏳ Model is loading or initializing...\n\n" \
                  + f"This happens on first use. Please try again in 30-60 seconds.\n\n" \
                  + f"💡 Quick alternative: Use GPT-4o or Claude-3.5-Sonnet (instant results!)"
            return
        ...
        if response.status_code == 200:
            try:
                ...
                yield f"💡 Or use GPT-4o/Claude-3.5-Sonnet for instant results!"
                
        elif response.status_code == 401 or response.status_code == 403:
            # Authentication error - need HF token
            yield debug_msg + f"🔑 Authentication Required!\n\n" \
                  + f"To use open-source models, you need a FREE Hugging Face token:\n\n" \
                  + f"1. Get token: https://huggingface.co/settings/tokens\n" \
                  + f"2. Add HF_TOKEN secret in Space Settings\n" \
                  + f"3. Factory reboot\n\n" \
                  + f"OR use GPT-4o/Claude-3.5-Sonnet instead (they work now!)"
        elif response.status_code == 503:
            # Service unavailable - model loading
            yield debug_msg + f"⏳ Model is currently loading (cold start)...\n\n" \
                  + f"This can take 30-60 seconds on first use.\n" \
                  + f"Please wait a minute and try again.\n\n" \
                  + f"💡 Quick solution: Use GPT-4o or Claude-3.5-Sonnet (no waiting!)"
        else:
            try:
            ...
            if "loading" in str(error_msg).lower():
                yield debug_msg + f"⏳ Model is loading... This may take 20-30 seconds. Please try again."
            else:
                yield debug_msg + f"❌ Error from Hugging Face (HTTP {response.status_code}):\n{error_msg}\n\n" \
                      + f"💡 Tip: Use GPT-4o or Claude-3.5-Sonnet for now (they're working!)"
        ...
        yield f"💡 Please use GPT-4o or Claude-3.5-Sonnet instead."
        ...
        yield f"💡 Tip: Use GPT-4o or Claude-3.5-Sonnet for reliable results!"
...
if __name__ == "__main__":
    ...
    if is_huggingface:
        ...
        print(f"🚀 Launching Python to C++ Code Optimizer on Hugging Face Spaces")
