# Human English Language Testing System 4

# AI System that tests humans English language reading and writing skills.
#
# In this version:
# - Users can select the model used for essay generation and grading
#   (Claude Haiku, GPT-4o Mini, Gemini Flash) — all via OpenRouter
...
MODELS = {
    "Claude Haiku (Anthropic)": "anthropic/claude-haiku-4-5",
    "GPT-4o Mini (OpenAI)": "openai/gpt-4o-mini",
    "Gemini Flash (Google)": "google/gemini-2.5-flash",
}
...
SYSTEM_PROMPT = [
    ...
    The essay can be structured as title and body. Only paragraphs allowed (at least 2 paragraphs), no headers or subheaders. Essay must be 200 words max. Answers must be from the essay.
    Correct answers that are not in the essay should be marked as incorrect.
    """,
    },
]

RESPONSE_FORMAT = """then return the essay and questions in this json format and only this json format: 
      {
        "essay": "essay text in markdown format",
        "questions": ["question 1", "question 2", "question 3"]
      }

      do not put the json format in a code block.
      """
...
def submit_answers(final_answer, state):
    ...
    state["messages"].append(
        {
            "role": "user",
            "content": "Please grade the essay. 70+ A, 60-69 B, 50-59 C, 40-49 D, 0-39 F. show grading\
              first. After \
              show review as thus: the question, user answer, correct answer if wrong. format as \
                markdown in a table format. Dont include extranous info or noise",
        }
    )
...
def start_examination(difficulty, topic, num_questions, model_name, state):
    if state["essay_text"]:
        return (
            state["essay_text"],
            state["questions"][state["current_question"]],
...
with gr.Blocks() as ui:
    ...
    start_btn.click(
        start_examination,
        inputs=[difficulty, topic, gr.State(value=4), model_selector, exam_state],
        outputs=[
            essay_text_container,
            question_text_container,
            analysis_container,
